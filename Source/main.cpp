...
void log(std::string t_actionDescription, NVSDK_NGX_Result t_result) {
  ...
  case NVSDK_NGX_Result_Success:
    log(t_actionDescription + ": Success");
    break;
  default:
...
extern "C" UNITY_INTERFACE_EXPORT void InitializeNGX() {
  NVSDK_NGX_Result result = NVSDK_NGX_VULKAN_Init(
      NVSDK_NGX_ENGINE_TYPE_CUSTOM, nullptr, nullptr, nullptr, nullptr);
...
SetDebugCallback(void (*t_debugFunction)(const char *)) {
  ...
  if (Unity::interceptedGraphicsInitialization)
...
interceptInitialization(PFN_vkGetInstanceProcAddr t_instance, void *user_data) {
  Vulkan::createInstance =
      (typeof(Vulkan::createInstance))t_instance(nullptr, "vkCreateInstance");
  return t_instance;
}
...
//  Unity::interceptedGraphicsInitialization =
//      Unity::vulkanGraphicsInterface->InterceptInitialization(
//          interceptInitialization, nullptr);
  Unity::interceptedGraphicsInitialization = true;
}
