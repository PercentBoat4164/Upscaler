...
namespace Upscaler.Runtime
{
    ...
    [AddComponentMenu("Rendering/Upscaler")]
    [ExecuteAlways]
    public class Upscaler : MonoBehaviour
    {
        ...
        public enum Quality
        {
            ...
            Auto,
            /// Render at native resolution and use the <see cref="Technique"/> for antialiasing. Not available for <see cref="Upscaler.IsSpatial(Upscaler.Technique)"/> <see cref="Technique"/>s.
            AntiAliasing,
        ...
        /// Maximum reactive value. More reactivity favors newer information. <c>0.6f</c> works well in our Unity testing scene, but please test for your specific title. Defaults to <c>0.6f</c>. Only used when <see cref="technique"/> is <see cref="Technique.FidelityFXSuperResolution"/>.
        public float reactiveMax = 0.6f;
        public float PreviousReactiveMax { get; private set; }
        /// Value used to scale reactive mask after generation. Larger values result in more reactive pixels. <c>0.9f</c> works well in our Unity testing scene, but please test for your specific title. Defaults to <c>0.9f</c>. Only used when <see cref="technique"/> is <see cref="Technique.FidelityFXSuperResolution"/>.
        public float reactiveScale = 0.9f;
        public float PreviousReactiveScale { get; private set; }
        /// Minimum reactive threshold. Increase to make more of the image reactive. <c>0.3f</c> works well in our Unity testing scene, but please test for your specific title. Defaults to <c>0.3f</c>. Only used when <see cref="technique"/> is <see cref="Technique.FidelityFXSuperResolution"/>.
         ...
         * <remarks>This is a convenience method for <see cref="IsSpatial(Upscaler.Technique)"/>. This
         ...
         * <summary>Is <see cref="technique"/> spatio-temporal?</summary>
         * <returns><c>true</c> if <see cref="technique"/> is spatio-temporal, <c>false</c> otherwise.</returns>
         * <remarks>This is a convenience method for <see cref="IsTemporal(Upscaler.Technique)"/>. This
         * is useful for determining whether motion vectors are required. <see cref="Technique"/>s for which this
         ...
         * <see cref="RequiresNativePlugin(Upscaler.Technique)"/>.</remarks>
         * <example><code>
            ...
            if (needsUpdate)
            {
                    ...
                    (technique == Technique.FidelityFXSuperResolution && autoReactive))
                {
                    if (_generateOpaque == null)
                    {
                        ...
                        _generateOpaque.name = "Upscaler | Copy Opaque";
                    ...
                    if (_cleanupOpaque == null)
                    {
                        ...
                        _cleanupOpaque.name = "Upscaler | Clean Up Opaque";
