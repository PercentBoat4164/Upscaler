...
Shader "Conifer/Upscaler/Snapdragon Game Super Resolution/V2Fragment2Pass"
{
    ...
    SubShader
    {
        ...
        Pass
		{
			...
			CGPROGRAM
			#pragma target 5.0
			#pragma multi_compile_local_fragment _ CONIFER__UPSCALER__ENABLE_OUTPUT_RESOLUTION_MOTION_VECTORS
			...
			float4 Frag(v2f_img input) : SV_Target
			{
				...
				return float4(Conifer_Upscaler_MotionVectors.Load(int3(input.uv * Conifer_Upscaler_OutputSize, 0)), depthclip, 0.0);
#else
				return float4(Conifer_Upscaler_MotionVectors.Load(int3(input.uv * Conifer_Upscaler_RenderSize, 0)), depthclip, 0.0);
#endif
			}
			ENDCG
		}
